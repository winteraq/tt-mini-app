...
baseComponent({
    ...
    classes: [
      ...
      function (prefixCls, btns, verticalButtons) {
        ...
        const button = btns.map((button) => {
          ...
          const hover =
            button.hoverClass && button.hoverClass !== 'default'
              ? button.hoverClass
  ...
  methods: {
    ...
    runCallbacks(e, method) {
      ...
      if (!button.disabled) {
        this.hide(
          () => typeof button[method] === 'function' && button[method](e)
        )
      }
    },
    /**
    ...
    /**
     * 用户点击该按钮时，会返回获取到的用户信息，回调的detail数据与tt.getUserInfo返回的一致，open-type="getUserInfo"时有效
     */
    bindgetuserinfo(e) {
              ...
              },
              {
                ...
                onTap: (e) => {
                  typeof opts.onConfirm === 'function' &&
                    opts.onConfirm(e, this.data.prompt.response)
                },
              },
            ],
          },
