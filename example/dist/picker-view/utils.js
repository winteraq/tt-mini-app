...
export function getRealIndexes(indexes = [], cols = []) {
  return cols.reduce(
    (acc, col, idx) => [...acc, getRealIndex(indexes[idx], 0, col.length - 1)],
    []
  )
}
...
export function getIndexFromValue(
  ...
  return getRealIndex(
    col.map((v) => v[fieldNames.value]).indexOf(value),
    0,
    col.length - 1
  )
}
...
export function getValueFromIndex(
  index,
  col = [],
  fieldNames = defaultFieldNames
) {
...
export function getValuesFromIndexes(
  ...
  return cols.reduce(
    (acc, col, idx) => [
      ...acc,
      getValueFromIndex(indexes[idx], col, fieldNames),
    ],
    []
  )
}
...
export function getRealValue(
  ...
  return col.length > 0
    ? col[getIndexFromValue(value, col, fieldNames)][fieldNames.value]
    : null
}
...
export function getRealValues(
  ...
  return cols.length > 0
    ? cols.reduce(
        (acc, col, idx) => [...acc, getRealValue(values[idx], col, fieldNames)],
        []
      )
    : []
}
...
export function getLabelsFromIndexes(indexes, cols = [], member) {
  return cols.reduce(
    (acc, col, idx) => [...acc, getLabelFromIndex(indexes[idx], col, member)],
    []
  )
}
