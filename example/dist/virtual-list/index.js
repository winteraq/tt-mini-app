...
baseComponent({
  ...
  methods: {
    ...
    onChange(scrollOffset, scrolled, callback) {
      ...
      const hasChanged =
        state.startIndex !== startIndex || state.endIndex !== endIndex

      // 计算起始点是否可视
      const direction = scrollOffset > this.data.scrollOffset ? 'Down' : 'Up'
      const firstItemVisible =
        direction === 'Up' && viewTop < startIndex * itemHeight
      const lastItemVisible =
        direction === 'Down' && viewTop > endIndex * itemHeight - height
    ...
    render(items, success) {
      ...
      this.getBoundingClientRect(() =>
        this.onChange(scrollOffset, true, success)
      )
    },
    /**
     * 滚动到指定的位置
    ...
    scrollTo(scrollOffset, success) {
      if (typeof scrollOffset === 'number') {
        const offset = Math.max(
          0,
          Math.min(scrollOffset, this.items.length * this.data.itemHeight)
        )
    ...
    setScrollHandler(_debounce = this.data.debounce) {
      this.scrollHandler = _debounce
        ? debounce(this.onScroll.bind(this), _debounce, false)
        : this.onScroll
    },
    /**
     * 阻止触摸移动
