...
function getFileString(snippet, opts) {
  ...
  var componentImports = extra
    .map(function (v) {
      return inspectFile(IMPORT_SNIPPET, v)
    })
    .join('\n')
  var componentAliases =
    `const version = '{{version}}'`.replace(/{{version}}/, version) + '\n'
  componentAliases =
    componentAliases +
    data
      .map(function (v) {
...
function getOptions(files) {
  ...
  var importFiles = exclude.filter(function (v) {
    return files.includes(v)
  })
  var exportFiles = files.filter(function (v) {
    return include.includes(v)
  })
  return {
...
function generateScript(files) {
  return through2.obj(function (file, encoding, cb) {
    ...
    if (file.isStream()) {
      this.emit(
        'error',
        new util.PluginError('generateScript', 'Streaming not supported')
      )
      return cb()
    }
...
function generateStyle(themes, options) {
  return through2.obj(function (file, encoding, cb) {
    ...
    if (file.isStream()) {
      this.emit(
        'error',
        new util.PluginError('generateStyle', 'Streaming not supported')
      )
      return cb()
    }
