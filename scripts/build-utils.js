...
export const clean = (dest) => () =>
  del([path.join(dest, 'lib'), path.join(dest, 'core'), path.join(dest, 'es')])
...
export const scripts = (paths, base) => () =>
  gulp
    .src(paths.src, { base, since: gulp.lastRun(scripts) })
    .pipe(babel())
    .pipe(uglify())
    .pipe(gulp.dest(paths.dest))
...
export const babelOnly = (paths, base) => () =>
  gulp
    .src(paths.src, { base, since: gulp.lastRun(babelOnly) })
    .pipe(babel())
    .pipe(gulp.dest(paths.dest))
