...
function getStyles(value) {
  return Array.isArray(value)
    ? value.map((n) => styleToCssString(n))
    : styleToCssString(value)
}
...
baseComponent({
  ...
  computed: {
    classes: [
      'prefixCls, labelAlign',
      function (prefixCls, labelAlign) {
  ...
  observers: {
    ...
    inputValue(newVal) {
      const { selectedIndex, selectedValue } = this.getValue(newVal)
  ...
  methods: {
    ...
    updated(inputValue, isForce) {
      ...
      // 设置选择器位置
      if (isForce) {
        this.select(inputValue, this.data.itemHeight, (y) =>
          this.scrollTo(y, 0, false)
        )
      }
    },
    ...
    getValue(value = this.data.inputValue, cols = this.data.cols) {
      ...
      const displayValue = getLabelFromIndex(
        selectedIndex,
        cols,
        fieldNames.label
      )
     ...
     * 设置选择器的位置信息
     */
    scrollTo(y, time = 0.3, runCallbacks = true) {
      if (this.scrollY !== y) {
    ...
    onFinish() {
      ...
      if (child && !child.disabled) {
        this.scrollTo(targetY, time < 0.3 ? 0.3 : time)
      } else {
        this.select(this.data.inputValue, itemHeight, (y) =>
          this.scrollTo(y, 0, false)
        )
      }
