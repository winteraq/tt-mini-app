...
baseComponent({
  ...
  computed: {
    classes: [
      'prefixCls, disabled, inputFocus, error, hasCount',
      function (prefixCls, disabled, inputFocus, hasError, hasCount) {
    ...
    updateHeight(val = this.data.rows) {
      ...
      if (inputRows !== rows) {
        wx.createSelectorQuery()
          .in(this)
          .select(`.${prefixCls}__item`)
          .boundingClientRect((rect) => {
            if (rect) {
              const lineHeight =
                inputRows > 1 ? rect.height / inputRows : rect.height
