  ...
  methods: {
    ...
    getMinDate() {
      return this.data.minDate
        ? valueToDate(this.data.minDate, this.data)
        : this.getDefaultMinDate()
    },
    getMaxDate() {
      return this.data.maxDate
        ? valueToDate(this.data.maxDate, this.data)
    ...
    getDate(d) {
      ...
      return this.clipDate(
        date ? valueToDate(date, this.data) : this.getMinDate()
      )
    },
    getDateData(date) {
      ...
      const minDay =
        minDateYear === selYear && minDateMonth === selMonth ? minDateDay : 1
      const maxDay =
        maxDateYear === selYear && maxDateMonth === selMonth
          ? maxDateDay
          : getDaysInMonth(date)
    ...
    getTimeData(date) {
      ...
      hours = [
        ...hours,
        ...fomartArray(minHour, maxHour).map((i) => ({
          value: i + '',
