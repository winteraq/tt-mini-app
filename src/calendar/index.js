...
// 获取元素旋转属性
const getTransform = (translate, isH) =>
  ...
  methods: {
    ...
    updateCurrentMonthYear(dir) {
      ...
      const currentMonth = parseInt(
        months[dir === 'next' ? months.length - 1 : 0].month,
        10
      )
      const currentYear = parseInt(
        months[dir === 'next' ? months.length - 1 : 0].year,
        10
      )
      const currentMonthName = monthNames[currentMonth]
    ...
    onTouchMove(e) {
      ...
      query
        .select(`.${prefixCls}__months-content`)
        .boundingClientRect((rect) => {
          ...
          this.touchesDiff = this.isH
            ? this.move.x - this.start.x
            : this.move.y - this.start.y
    ...
    onMonthChangeStart(dir) {
      ...
      if (typeof this.fns.onMonthYearChangeStart === 'function') {
        this.fns.onMonthYearChangeStart.call(
          this,
          params.currentYear,
          params.currentMonth
        )
    ...
    onMonthChangeEnd(dir = 'next', rebuildBoth = false) {
      ...
      if (!rebuildBoth) {
        ...
        prevMonthHTML = this.monthHTML(
          new Date(currentYear, currentMonth),
          'prev'
        )
        nextMonthHTML = this.monthHTML(
          new Date(currentYear, currentMonth),
          'next'
        )
        months = [
      ...
      this.$$setData({ months, monthsTranslate }).then(
        () => (this.isRendered = false)
      )

      if (typeof this.fns.onMonthAdd === 'function') {
        this.fns.onMonthAdd.call(
          this,
          dir === 'next' ? months[months.length - 1] : months[0]
        )
      }
    ...
    setWeekHeader() {
      ...
      if (weekHeader) {
        for (let i = 0; i < 7; i++) {
          const weekDayIndex =
            i + firstDay > 6 ? i - 7 + firstDay : i + firstDay
    ...
    monthHTML(date, offset) {
      ...
      let daysInPrevMonth = this.daysInMonth(
      ...
      let dayDate,
        ...
        minDate = this.data.minDate
          ? new Date(this.data.minDate).getTime()
          : null,
        maxDate = this.data.maxDate
          ? new Date(this.data.maxDate).getTime()
          : null
      ...
      this.data.months.forEach((n, i) => {
        n.items.forEach((v, k) => {
          v.forEach((p, j) => {
            if (p.type.selected) {
              changedPath[
                `months[${i}].items[${k}][${j}].type.selected`
              ] = false
            }
          })
        })
