...
const defaultPlatformProps = {
  labelPropName: 'label',
  format(values, props) {
    return Array.isArray(values.displayValue)
      ? values.displayValue.join(',')
      : values.displayValue
  },
}
...
export default function popupMixin(
  selector = '#wux-picker',
  platformProps = defaultPlatformProps
) {
  return Behavior({
    ...
    methods: {
      ...
      close(callback) {
        if (typeof callback === 'function') {
          const values = this.getPickerValue(
            this.scrollValue || this.data.inputValue
          )
      ...
      formatPickerValue(values) {
        return {
          ...values,
          [platformProps.labelPropName]: platformProps.format(
            values,
            this.data
          ),
        }
      ...
      setChildProps() {
        ...
        if (elements.length > 0) {
          elements.forEach((inputElem) => {
            ...
            const oriInputEvents = inputElem.data.oriInputEvents || {
              ...inputEvents,
            }
      ...
      setCasecadeScrollValue(value) {
        if (value && this.scrollValue) {
          ...
          if (
            length === value.length &&
            this.scrollValue[length - 1] === value[length - 1]
          ) {
            return
          }
        }
