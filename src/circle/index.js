  ...
  methods: {
    ...
    draw(line = true) {
      ...
      const startAngle = counterclockwise
        ? p - this.data.beginAngle
        : this.data.beginAngle
      const endAngle = counterclockwise
        ? p - (this.data.beginAngle + this.data.currentAngle)
        : this.data.beginAngle + this.data.currentAngle

      // 创建 canvas 绘图上下文
      this.ctx = this.ctx || tt.createCanvasContext('circle', this)
    ...
    animate(c, d, e) {
      ...
      const isEnd =
        (!e && 1000 * this.data.currentAngle <= Math.floor(1000 * endAngle)) ||
        (e && 1000 * this.data.currentAngle >= Math.floor(1000 * endAngle))
